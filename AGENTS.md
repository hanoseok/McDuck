...
- **앱 내 "Add to Claude Code" 등록(설정 패널):** 헤더 기어 설정의 버튼이 플러그인을 등록·활성화합니다(`PluginInstaller`). 1순위로 `claude` CLI(`plugin marketplace add <번들경로>` → `plugin install mcduck@mcduck`)를 시도하고, **실패하면 `~/.claude/settings.json`에 직접 기록**(fallback): `extraKnownMarketplaces.mcduck = { source: { source: "directory", path: <ClaudePlugin 절대경로> } }` + `enabledPlugins["mcduck@mcduck"] = true`(다른 키는 보존 병합). 적용은 Claude Code 재시작/`/reload-plugins`. 핵심 로직(JSON 병합·오케스트레이션)은 `Tests/McDuckTests`로 커버.
