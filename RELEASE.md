...
- **메뉴바 라벨(기간 선택)**: 메뉴바 아이콘과 함께 선택한 기간의 토큰(위)·비용(아래, 소수점 없음)을 2줄로 표시합니다(`ImageRenderer`로 렌더링해 바 높이에 맞춤). 헤더 기어 설정에서 **None / Today / Week / Month / Total**을 선택(UserDefaults에 저장)하며 기본값은 Today입니다.
